...
switch source
    case "OSU"
        ...
        for n=1:length(alpha_cycles)
            h_exp_raw(n) = plot(alpha_cycles{n},coef_cycles{n},'bo','LineWidth',plot_opt.lw,'MarkerSize',plot_opt.ms,'Parent',ax);
        end
    ...
    otherwise
        ...
        elseif length(authors) == 1 || isnan(alpha_mod(1))   
            h = plot(alpha_plot,coef_plot,'k-',alpha_exp,coef_exp,'bo','LineWidth',plot_opt.lw,'MarkerSize',plot_opt.ms,'Parent',ax);
        elseif length(authors) == 2
            h = plot(alpha_plot,coef_plot,'k-',alpha_exp,coef_exp,'bo',alpha_mod,coef_mod,'k--','LineWidth',plot_opt.lw,'MarkerSize',plot_opt.ms,'Parent',ax);
        end
end
