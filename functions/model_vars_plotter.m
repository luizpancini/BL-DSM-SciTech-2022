function model_vars_plotter(interp,outputs,source,model,tabgp,plot_opt,p)
...
ylabel('Separation points','FontWeight','normal','FontSize',plot_opt.axes_size);
...
% Find interpolated values
c_mf_interp = cm_ref-c_mv(range)-c_mI(range)-p.c_m0; % Estimated value for c_mf from the experiment (assuming the model's c_mv and c_mI are true)
% Newton-Raphson iterations to find experimental f2prime_m
f2prime_m_interp = nan(ittf-itt+1,1);
for i=1:ittf-itt+1
    if i == 1, f_guess = 0.5; else, f_guess = f2prime_m_interp(i-1); end
    j = i-1+itt;
    f2prime_m_interp(i) = fsolve(@(x) find_f2prime_m(x,c_mf_interp(i),c_nf(j),R(j),RD(j),S(j),theta(j),q(j),p.kappa_0,p.kappa_1,p.kappa_2,p.kappa_3,p.K0,p.K1,p.K2),f_guess,optimoptions('fsolve','Display','off')); % Solve transcendental equation for f2prime_m
...
f2prime_c_interp = nan(N_range,1);
alpha_ratio = abs(alpha_lag)./(p.alpha1_0c);
for i=1:ittf-itt+1
    if alpha_ratio(i-1+itt) < 1
        f2prime_c_interp(i) = real(((cc_ref(i)+p.c_d0*cos(alpha_ref(i)))/((1-p.eta*RD(i-1+itt)^2)*p.c_n_alpha*sin(alpha_C(i-1+itt)-p.alpha_0L)^2)) ^ (1/(1+alpha_ratio(i-1+itt))));
    else
        f2prime_c_interp(i) = real(((cc_ref(i)+p.c_d0*cos(alpha_ref(i))+p.E1*min([1,alpha_ratio(i-1+itt)^3-1]))/((1-p.eta*RD(i-1+itt)^2)*p.c_n_alpha*sin(alpha_C(i-1+itt)-p.alpha_0L)^2)) ^ (1/(1+alpha_ratio(i-1+itt))));
    end
end
...
%% Angle offsets
tab4 = uitab('Parent',tabgp,'Title','d_a','BackgroundColor',[1 1 1]);
ax4 = axes('Parent',tab4,'FontSize',plot_opt.axes_size,'FontName','times new roman');
ax4.XLim = XLim;
hold(ax4,'on'); 
plot(alpha(iti:end)*180/pi,dalpha_n(iti:end)*180/pi,'k-',alpha(iti:end)*180/pi,dalpha_m(iti:end)*180/pi,'b-',alpha(iti:end)*180/pi,dalpha_c(iti:end)*180/pi,'g-','LineWidth',plot_opt.lw,'Parent',ax4);
xlabel('$\alpha$ [deg]','FontWeight','normal','FontSize',plot_opt.axes_size);
ylabel('$\Delta\alpha$ [deg]','FontWeight','normal','FontSize',plot_opt.axes_size);
legend('Normal','Moment','Chordwise');
set(legend,'Location','best','FontSize',plot_opt.axes_size*0.6);
grid on

%% Stall onset ratio
tab5 = uitab('Parent',tabgp,'Title','theta','BackgroundColor',[1 1 1]);
ax5 = axes('Parent',tab5,'FontSize',plot_opt.axes_size,'FontName','times new roman');
ax5.XLim = XLim;
hold(ax5,'on'); 
plot(alpha(iti:end)*180/pi,theta(iti:end),'k-',alpha(iti:end)*180/pi,R(iti:end),'b-',alpha(iti:end)*180/pi,RD(iti:end),'r-',alpha(iti:end)*180/pi,RD(iti:end).^p.lambda_2,'m-',alpha(iti:end)*180/pi,RD_theta(iti:end),'g-','LineWidth',plot_opt.lw,'Parent',ax5);
xlabel('$\alpha$ [deg]','FontWeight','normal','FontSize',plot_opt.axes_size);
ylabel('$\theta$','FontWeight','normal','FontSize',plot_opt.axes_size);
R_prime_to_lambda2 = "$R^{\prime" + num2str(p.lambda_2) + "}$";
legend('$\theta$','$R$','$R^{\prime}$',R_prime_to_lambda2,'$R^{\prime}_{\theta}$');
set(legend,'Location','best','FontSize',plot_opt.axes_size*0.6);
grid on
